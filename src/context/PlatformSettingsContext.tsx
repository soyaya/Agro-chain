...
const PlatformSettingsContext = createContext<PlatformSettingsContextValue>({
  ...
  getPricePerUnit: (fishType, weightKg) => {
    const price =
      FALLBACK_PRICES_PER_KG[fishType as FishType] ??
      FALLBACK_PRICES_PER_KG.table_size;
...
export function PlatformSettingsProvider({
  ...
  const [pricePerKg, setPricePerKg] = useState<FishPriceConfig>(
  ...
  const getPricePerUnit = (fishType: string, weightKg: number): number => {
    const pricePerKgForType =
      pricePerKg[fishType as FishType] ?? pricePerKg.table_size;
    return weightKg * pricePerKgForType;
  };

  return (
    <PlatformSettingsContext.Provider
      value={{ pricePerKg, isLoaded, getPricePerUnit }}
    >
      {children}
    </PlatformSettingsContext.Provider>
  );
