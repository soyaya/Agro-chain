...
export default function SupplyAdminClusterFarmersPage() {
  ...
  const handleInvite = async () => {
    if (!form.fullName || !form.phone || !form.email || !form.locationLga) {
      toast.error("Fill in all fields to invite a cluster farmer.");
      return;
    }
    setInviting(true);
    try {
  ...
  const clusterFarmers = team.filter((m) => m.role === "cluster");

  if (loading) return <LoadingState message="Loading cluster farmers..." size="lg" />;

  return (
    <div className="flex flex-col gap-(--section-gap)">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h1 className="font-ubuntu mb-2 text-3xl font-bold text-(--heading-colour)">Cluster Farmers</h1>
        <p className="font-roboto-slab text-(--text-colour)">
          Invite cluster farmers to help run your region — they're auto-approved on invite and can
          review farmer listings and riders themselves right away.
        </p>
      </motion.div>
      ...
      {clusterFarmers.length > 0 ? (
        ...
        <EmptyState
          icon={Users}
          title="No cluster farmers yet"
          description="Invite your first cluster farmer to start reviewing listings and riders in your region."
          size="lg"
        />
      )}
    </div>
