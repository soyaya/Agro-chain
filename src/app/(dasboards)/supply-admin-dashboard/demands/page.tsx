...
import {
  supplyAdminService,
} from "~/lib/services/supply-admin.service";
import type { BackendDemand, DemandStatus, ApprovedRider } from "~/lib/services/cluster.service";
...
const FULFILLMENT_STAGE_LABELS: Record<string, string> = {
  ...
  at_cluster_office: "Sourced — ready to hand off",
          ...
          >
            <div className="flex flex-col gap-(--gap-base)">
              <div className="flex items-center gap-(--gap-base)">
                ...
                <div>
                  <h3 className="font-ubuntu text-xl font-bold text-(--heading-colour)">Decline Demand</h3>
...
export default function SupplyAdminDemandsPage() {
  ...
  const handleAccept = async (id: string) => {
    setActionLoading(id);
    try {
      await supplyAdminService.acceptDemand(id);
  ...
  const handleDeclineConfirm = async (reason: string) => {
    ...
    try {
      await supplyAdminService.declineDemand(selectedId, reason || undefined);
  ...
  const handleReadyForPickup = async (id: string) => {
    setActionLoading(id);
    try {
      await supplyAdminService.markDemandReadyForPickup(id);
  ...
  const handleEscalate = async (riderId: string) => {
    ...
    try {
      await supplyAdminService.escalateDemandToRider(escalateDemandId, riderId);
    ...
    <div className="flex flex-col gap-(--section-gap)">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        ...
        <p className="font-roboto-slab text-(--text-colour)">
          Buyer demand routed to you from outside your region. Accept to source it yourself, then
          hand it off for pickup or delivery — the buyer confirms when it arrives, and you're paid
          per your standing supply contract.
        </p>
      </motion.div>
      ...
      >
        {(["all", "assigned", "accepted", "declined", "fulfilled"] as const).map((status) => (
          ...
          >
            ...
            <span className="font-roboto-slab text-xs text-(--text-colour) capitalize">{status}</span>
          </button>
        ))}
      </motion.div>
      ...
      {filtered.length > 0 ? (
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="font-roboto-slab text-sm text-(--text-colour)">from {demand.buyerName}</p>
                  </div>
                  <span
        ...
        <EmptyState
          ...
          description={
            filterStatus === "all"
              ? "No demands have been routed to you yet. Admin assigns out-of-region buyer demand to you based on your coverage."
