...
export default function CreateClusterListingPage() {
  ...
  const handleSubmit = async (data: SupplyListingFormData) => {
    setIsLoading(true);
    try {
      // Same endpoint the farmer flow uses — the backend auto-approves a
      // listing created by a cluster-role account instead of routing it to
      // pending approval, since the cluster farmer is already the approver.
          ...
          <motion.div variants={FADE_IN_VARIANT} className="flex flex-col gap-(--gap-base)">
            ...
            <div>
              ...
              <p className="mt-2 text-(--text-colour)">
                List your aggregated fish supply — published to the marketplace immediately.
              </p>
            </div>
          </motion.div>
