...
import { DollarSign, Receipt, TrendingUp, ArrowRight, Clock, Users } from "lucide-react";
...
export default function ClusterFinancialPage() {
  ...
  const stats = [
    { label: "Total Cluster Earnings", value: `₦${totalEarnings.toLocaleString()}`, icon: DollarSign, color: "text-green-600", bg: "bg-green-50", note: "All time" },
    { label: "Pending Payouts", value: `₦${pendingPayouts.toLocaleString()}`, icon: Clock, color: "text-yellow-600", bg: "bg-yellow-50", note: "Awaiting release" },
    { label: "Total Payouts", value: payouts.length.toString(), icon: Receipt, color: "text-blue-600", bg: "bg-blue-50", note: "Recorded" },
  ];
          ...
          return (
            ...
            >
              <div className="mb-(--space-lg) flex items-center justify-between">
                ...
                <span className="font-roboto-slab text-xs text-gray-400">{s.note}</span>
              </div>
      ...
      >
        ...
        {payouts.length === 0 ? (
          <p className="font-roboto-slab py-(--space-lg) text-center text-sm text-gray-500">
            No payouts recorded yet. Earnings appear here once orders are fulfilled and confirmed.
          </p>
        ) : (
