...
function BarChart({ title, rows }: { title: string; rows: { label: string; count: number }[] }) {
  ...
  return (
    ...
    >
      <h2 className="font-ubuntu text-heading-colour mb-5 text-xl font-semibold">{title}</h2>
      {rows.length === 0 ? (
        <p className="font-roboto-slab text-text-colour text-sm">No data yet.</p>
      ) : (
          ...
          {rows.map((row, i) => (
            <div key={row.label} className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                ...
                <span className="font-roboto-slab text-text-colour text-sm">{row.count}</span>
              </div>
  ...
  return (
    <div className="flex flex-col gap-(--section-gap)">
      <motion.div variants={SLIDE_UP_VARIANT} initial="hidden" animate="visible">
        <h1 className="font-ubuntu text-heading-colour text-3xl font-bold">Analytics</h1>
