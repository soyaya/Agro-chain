  ...
  return (
    <footer className="font-openSans relative">
      <div className="border-gray-border px-section-px sm:px-section-px-sm lg:px-section-px-lg mx-auto grid h-full w-full max-w-7xl gap-8 rounded-lg border-3">
        ...
        {/* Footer 2 */}
        <div className="text-text-colour flex flex-col items-center justify-between gap-2 py-2 lg:flex-row">
            ...
            {footer2Links.map(({ label, href, icon: Icon }) => {
              return (
                <li key={href}>
                  <Link
                    href={href}
                    className="hover:text-heading-colour text-sm focus:text-(--heading-color) lg:text-lg"
                  >
                    {Icon && <Icon className="h-4 w-4" />}
                    <span>{label}</span>
                  </Link>
