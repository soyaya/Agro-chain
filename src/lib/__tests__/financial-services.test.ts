...
describe("Financial Services Utilities", () => {
  ...
  describe("isFinancialFeatureEnabled", () => {
    it("should return false for regular dashboard config", () => {
      expect(isFinancialFeatureEnabled(buyerDashboardConfig, "loans")).toBe(
        false,
      );
      expect(isFinancialFeatureEnabled(buyerDashboardConfig, "credit")).toBe(
        false,
      );
    });
    ...
    it("should return true for enabled features in enhanced config", () => {
      expect(
        isFinancialFeatureEnabled(enhancedFarmerDashboardConfig, "loans"),
      ).toBe(true);
      expect(
        isFinancialFeatureEnabled(enhancedFarmerDashboardConfig, "credit"),
      ).toBe(true);
      expect(
        isFinancialFeatureEnabled(enhancedFarmerDashboardConfig, "payments"),
      ).toBe(true);
      expect(
        isFinancialFeatureEnabled(enhancedFarmerDashboardConfig, "profile"),
      ).toBe(true);
    });
  });
  ...
  describe("getFinancialNavigationItems", () => {
    ...
    it("should return financial navigation items for enhanced config", () => {
      ...
      expect(items.some((item) => item.label === "Loan Applications")).toBe(
        true,
      );
      expect(items.some((item) => item.label === "Credit Purchases")).toBe(
        true,
      );
      expect(items.some((item) => item.label === "Payment History")).toBe(true);
      expect(items.some((item) => item.label === "Financial Profile")).toBe(
        true,
      );
    });
  });
  ...
  describe("shouldShowFinancialServices", () => {
    it("should return true for farmer dashboard paths", () => {
      ...
      expect(shouldShowFinancialServices("/farmers-dashboard/profile")).toBe(
        true,
      );
    });
    ...
    it("should return true for cluster dashboard paths", () => {
      ...
      expect(shouldShowFinancialServices("/cluster-dashboard/farmers")).toBe(
        true,
      );
    });
