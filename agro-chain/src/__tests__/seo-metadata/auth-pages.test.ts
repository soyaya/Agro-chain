...
import { metadata as verifyIdentityMetadata } from "../../app/(auth)/verify-identity/layout";
    ...
    fc.assert(
      fc.property(
        ...
        (index) => {
          ...
          expect(og!.title, `${name}: openGraph.title should be defined`).toBeDefined();
          expect(typeof og!.title, `${name}: openGraph.title should be a string`).toBe("string");
          expect((og!.title as string).length, `${name}: openGraph.title should not be empty`).toBeGreaterThan(0);

          // Non-empty openGraph.description
          expect(og!.description, `${name}: openGraph.description should be defined`).toBeDefined();
          expect(typeof og!.description, `${name}: openGraph.description should be a string`).toBe("string");
          expect((og!.description as string).length, `${name}: openGraph.description should not be empty`).toBeGreaterThan(0);

          // Non-empty openGraph.url
          expect(og!.url, `${name}: openGraph.url should be defined`).toBeDefined();
          expect(typeof og!.url, `${name}: openGraph.url should be a string`).toBe("string");
          expect((og!.url as string).length, `${name}: openGraph.url should not be empty`).toBeGreaterThan(0);

          // keywords must be absent
          expect(meta.keywords, `${name}: keywords should be absent on auth pages`).toBeUndefined();
        }
      ),
      { numRuns: 100 }
    );
